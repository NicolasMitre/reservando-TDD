...
Listado.prototype.buscarRestaurante = function(id) {
  const restauranteSelect = this.restaurantes.find(
    restaurantes => restaurantes.id === id
  );
...
Listado.prototype.obtenerUbicaciones = function() {
  ...
  const ubicacionesArray = this.restaurantes.map(function(restaurante) {
    return restaurante.ubicacion;
  });
  //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
  var nuevoArrayUbicaciones = eliminarRepetidosDelArray(ubicacionesArray);

  return nuevoArrayUbicaciones.sort();
};
...
Listado.prototype.obtenerRubros = function() {
  const rubrosArray = this.restaurantes.map(function(restaurante) {
    return restaurante.rubros;
  });

  var nuevoArrayRubros = eliminarRepetidosDelArray(rubrosArray);

  return nuevoArrayRubros.sort();
};
...
Listado.prototype.obtenerHorarios = function() {
  ...
  const horariosArray = this.restaurantes.map(function(restaurante) {
    return restaurante.horarios;
  });

  //En este arreglo vamos a poner todos los horarios, uno por uno
  horariosArray.forEach(function(arreglo) {
  ...
  var nuevoArrayHorarios = eliminarRepetidosDelArray(horariosArray);

  return nuevoArrayHorarios.sort();
};

function eliminarRepetidosDelArray(elemento) {
  elemento.filter(function(elem, index, self) {
